...
export class TourQueryService {
    ...
    return this.prisma.tour.update({
      ...
      data: {
        ...
        ...(rest.duration !== undefined && {
          duration: rest.duration?.trim() || 'Chưa xác định',
        }),
  ...
  async restoreTour(id: number) {
    ...
    if (!tour)
      throw new NotFoundException(`Không tìm thấy tour #${id} trong thùng rác`);
    return this.prisma.tour.update({
      where: { id },
      data: { deletedAt: null },
    });
  ...
  private async assertCanPermanentDeleteTour(id: number) {
    ...
    if (tour._count.bookings > 0) {
      throw new BadRequestException(
        'Tour đã phát sinh booking, chỉ được lưu trữ và khôi phục',
      );
    }
    return tour;
  }
  ...
  async getSaleDeals(localeInput?: string) {
    ...
    return departures.map((dep) => {
      ...
      return {
        ...
        duration:
          locale === 'en'
            ? tour.durationEn?.trim() ||
              toEnglishNameFallback(tour.duration, tour.duration)
            : tour.duration,
        newPrice: salePrice,
