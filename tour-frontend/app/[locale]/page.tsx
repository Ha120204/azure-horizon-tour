      ...
      <header className="relative pt-32 pb-20 px-8">
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <span className="inline-block px-4 py-1.5 rounded-full bg-secondary-fixed text-on-secondary-fixed text-[0.6875rem] font-label font-bold tracking-[0.05em] uppercase mb-6">{t('hero.badge')}</span>
              <h1 className="text-7xl font-headline font-extrabold text-on-surface tracking-tight leading-[1.1] mb-8">
                {t('hero.title1')} <br /><span className="text-primary italic">{t('hero.title2')}</span>.
              </h1>
            ...
            <div className="relative">
              ...
              <div className="absolute -bottom-6 -left-6 bg-surface-container-lowest p-6 rounded-xl ambient-shadow hidden md:block">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-headline font-bold">{t('footer.carbonNeutral')}</p>
                    <p className="text-xs text-on-surface-variant">{t('footer.sustainable')}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div key={tour.id} className="group flex flex-col bg-surface-container-lowest rounded-xl overflow-hidden hover:bg-white hover:ambient-shadow transition-all duration-300 cursor-pointer" onClick={() => router.push(`/tour/${tour.id}`)}>
                ...
                <div className="p-8 flex-1 flex flex-col">
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-outline-variant/10">
                    <div>
                      <span className="text-[0.625rem] font-label font-bold text-outline uppercase block tracking-widest mb-1">{t('featured.perPerson')}</span>
