            ...
            <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div className="mt-2">
                                <div className="flex gap-1 mb-1">
                                    {[1, 2, 3, 4, 5].map(index => (
                                        <div
                                            key={index}
                                            className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${index <= strength.score ? strength.color : 'bg-slate-200'}`}
                                        />
                                    ))}
                                </div>
                    ...
                    <PasswordField
                        ...
                        valid={Boolean(pwForm.confirmPassword && pwForm.confirmPassword === pwForm.newPassword)}
...
function PasswordField({
    ...
    return (
        <FieldInput label={label} id={id} error={error}>
            <div className="relative">
                <span className="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 text-[18px] pointer-events-none">{icon}</span>
                <input
                    id={id}
                    ...
                    className={`w-full bg-white border rounded-xl pl-11 pr-12 py-3 text-sm font-medium text-slate-800 outline-none transition-all focus:border-blue-400 focus:ring-2 focus:ring-blue-100 ${error ? 'border-red-400' : valid ? 'border-emerald-400' : 'border-slate-200'}`}
                />
                <button
                    type="button"
                ...
                >
                    <span className="material-symbols-outlined text-[19px]">{visible ? 'visibility_off' : 'visibility'}</span>
                </button>
                {valid && (
                ...
                {[
                    ...
                    return (
                        <div key={text} className="flex items-center gap-1.5">
                            <span className={`material-symbols-outlined text-[13px] ${password ? passed ? 'text-emerald-500' : 'text-slate-400' : 'text-slate-300'}`} style={{ fontVariationSettings: "'FILL' 1" }}>
                                {password && passed ? 'check_circle' : 'radio_button_unchecked'}
                            </span>
                            <span className={`text-[11px] font-medium ${password ? passed ? 'text-emerald-700' : 'text-slate-500' : 'text-slate-400'}`}>{text}</span>
                        </div>
                    );
                })}
            </div>
