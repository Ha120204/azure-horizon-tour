...
export type TourFilterSelectOption = {
    value: string;
    label: string;
    description?: string;
    icon?: string;
...
type TourFilterSelectProps = {
    value: string;
    options: TourFilterSelectOption[];
        ...
        <div ref={selectRef} className={`relative ${className}`}>
            ...
            >
                <span className="flex min-w-0 items-center gap-2.5">
                    ...
                    <span className={`truncate ${selectedOption ? '' : 'text-on-surface-variant'}`}>
            ...
            {isOpen && (
                <div
                    role="listbox"
                    className={`absolute left-0 right-0 z-[95] mt-2 min-w-[280px] overflow-hidden rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-1.5 shadow-2xl shadow-slate-900/12 ${menuClassName}`}
                >
