...
import {
    ...
    REFUND_PRESENTATIONS,
    getBookingPresentationKey,
} from '@/lib/booking/bookingStatus';

const PLACEHOLDER_IMAGE = 'https://images.unsplash.com/photo-1499681404123-6c7102ce0033';
...
export function BookingCard({ booking, isCompleted }: Props) {
    ...
    const refundPresentation =
        ...
        booking.refundStatus !== 'NONE'
            ? REFUND_PRESENTATIONS[booking.refundStatus]
            : null;
    ...
    useEffect(() => {
        ...
        if (!eligible) return;
        const expiryTime = new Date(booking.holdExpiresAt as string).getTime();
    ...
    }, [booking.status, booking.paymentStatus, booking.holdExpiresAt]);
    ...
    return (
        ...
        >
            <div className="relative aspect-[16/9] overflow-hidden bg-slate-100 md:aspect-auto md:min-h-[300px]">
                <Image
                    alt={booking.tour?.name || t('my_bookings.tourImageAlt')}
                    src={booking.tour?.imageUrl || PLACEHOLDER_IMAGE}
                    fill
                ...
                <span className="absolute bottom-3 left-3 max-w-[calc(100%-1.5rem)] truncate rounded-md bg-white/95 px-2.5 py-1.5 text-[11px] font-extrabold uppercase text-slate-800 shadow-sm">
                    {t('my_bookings.tourCode')} {booking.tour?.tourCode || t('my_bookings.notUpdated')}
                </span>
            </div>
            ...
            <div className="flex min-w-0 flex-col p-5 sm:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                        ...
                        <h2 className="break-words font-headline text-xl font-extrabold leading-tight text-on-surface transition-colors motion-reduce:transition-none group-hover:text-primary sm:text-2xl">
                            {booking.tour?.name || t('my_bookings.tourNameFallback')}
                        </h2>
                    </div>
                ...
                {refundPresentation && (
                    <div className={`mt-2 flex items-center gap-2 text-sm font-semibold ${refundPresentation.className}`}>
                        <span className="material-symbols-outlined text-base">
                            {refundPresentation.icon}
                        </span>
                        <span>
                            {refundPresentation.showAmount
                                ? t(refundPresentation.labelKey, { amount: formatPrice(booking.refundAmount ?? 0) })
                                : t(refundPresentation.labelKey)}
                        </span>
                    </div>
                )}
