...
export const BOOKING_PRESENTATIONS: Record<BookingPresentationKey, BookingPresentation> = {
    ...
    PAYMENT_EXPIRED: {
        badgeLabelKey: 'my_bookings.statusPaymentExpired',
        badgeIcon: 'timer_off',
        badgeClass: 'bg-slate-100 text-slate-600 border-slate-200',
        messageKey: 'my_bookings.messagePaymentExpired',
        messageIcon: 'schedule',
...
export const REFUND_PRESENTATIONS: Record<
    Exclude<CustomerRefundStatus, 'NONE'>,
    RefundPresentation
> = {
    REQUESTED: {
        labelKey: 'my_bookings.refundRequested',
        icon: 'hourglass_top',
        className: 'text-orange-700',
        showAmount: false,
    },
    PROCESSING: {
        labelKey: 'my_bookings.refundProcessing',
        icon: 'currency_exchange',
        className: 'text-sky-700',
        showAmount: true,
    },
    REFUNDED: {
        labelKey: 'my_bookings.refundRefunded',
        icon: 'paid',
        className: 'text-emerald-700',
        showAmount: true,
    },
    NOT_REQUIRED: {
        labelKey: 'my_bookings.refundNotRequired',
        icon: 'info',
...
export function getBookingPresentationKey(booking: {
    ...
    if (booking.isExpired) return 'PAYMENT_EXPIRED';
